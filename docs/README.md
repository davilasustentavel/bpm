...
**O acesso ao mapa de processos é restrito.** É necessário fazer login com uma conta `@davila.eco.br` para visualizar e operar o sistema.
...
| Perfil | Visualizar | Criar/Editar atividades | Criar/Editar/Excluir grupos e processos | Excluir atividades | Gerenciar pessoas |
|---|:---:|:---:|:---:|:---:|:---:|
| **Sem login** | ✗ | ✗ | ✗ | ✗ | ✗ |
| **Usuário ativo** | ✓ | ✗ | ✗ | ✗ | ✗ |
| **Editor** | ✓ | ✓ | ✗ | ✗ | ✗ |
...
> Nenhum usuário não-administrador pode excluir registros do sistema.
...
Ao entrar, o nome do usuário aparece no cabeçalho e os controles de edição disponíveis para o seu perfil ficam visíveis. Para sair, clique em **Sair**.

Usuários sem login veem apenas uma tela de acesso restrito — nenhum dado é exibido.

### Segurança no banco de dados

O banco utiliza **Row Level Security (RLS)** com as seguintes regras:

- **Leitura** — requer usuário autenticado com `ativo = true`
- **Escrita em grupos e processos** — requer usuário com `admin = true`
- **Escrita em atividades** — requer usuário com `admin = true` ou `editor = true`
- **Exclusão de atividades** — requer usuário com `admin = true`
...
> Os controles de edição abaixo só aparecem para os perfis que têm permissão para cada ação. Grupos e processos são exclusivos de **administradores**; atividades podem ser criadas e editadas por **editores** e administradores.
...
| **Usuário habilitado** | Quando marcado, a pessoa pode fazer login no sistema |
| **Editor de atividades** | Quando marcado, a pessoa pode criar e editar atividades (grupos e processos são exclusivos de admins). Para administradores este campo é sempre marcado e não editável |
| **Administrador** | Quando marcado, a pessoa tem acesso total ao sistema, incluindo gerenciar a tabela de pessoas |
